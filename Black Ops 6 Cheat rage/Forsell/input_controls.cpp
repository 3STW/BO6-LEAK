...
int hitmarkersounds;
int airstuck;
int overrideaimbotkey;

int anti_aim_yaw_key;
...
bool KeyBind::isPressed() const noexcept
{
    ...
    return static_cast<std::size_t>(keyCode) < keyMap.size() && ImGui::IsKeyPressed((ImGuiKey)keyMap[keyCode].code, false);
}
...
bool KeyBind::isDown() const noexcept
{
    ...
    return static_cast<std::size_t>(keyCode) < keyMap.size() && ImGui::IsKeyDown((ImGuiKey)keyMap[keyCode].code);
}

bool KeyBind::setToPressedKey() noexcept
{
    if (ImGui::IsKeyPressed((ImGuiKey)ImGui::GetIO().KeyMap[ImGuiKey_Escape])) {
        keyCode = KeyCode::NONE;
        return true;
    }
    ...
    for (int i = 0; i < IM_ARRAYSIZE(ImGui::GetIO().KeysDown); ++i) {
        ...
        if (const auto it = std::ranges::find(keyMap, i, &Key::code); it != keyMap.end()) {
            keyCode = static_cast<KeyCode>(std::distance(keyMap.begin(), it));
            // Treat AltGr as RALT
            if (keyCode == KeyCode::LCTRL && ImGui::IsKeyPressed((ImGuiKey)keyMap[KeyCode::RALT].code))
