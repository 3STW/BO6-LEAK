...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    if (g.IO.KeyShift)
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            ...
            GetForegroundDrawList()->AddText(text_pos, IM_COL32(255, 255, 255, 2555), buf, NULL);
            GetForegroundDrawList()->AddRect(merge_group->ClipRect.Min, merge_group->ClipRect.Max, IM_COL32(255, 255, 255, 2555));
        }
#endif
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    if (IsItemHovered())
        GetForegroundDrawList()->AddRect(table->OuterRect.Min, table->OuterRect.Max, IM_COL32(255, 255, 255, 2555));
    if (IsItemVisible() && table->HoveredColumnBody != -1)
        GetForegroundDrawList()->AddRect(GetItemRectMin(), GetItemRectMax(), IM_COL32(255, 255, 255, 2555));
    if (!open)
        return;
    if (table->InstanceCurrent > 0)
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        if (IsItemHovered())
        {
            ...
            GetForegroundDrawList()->AddRect(r.Min, r.Max, IM_COL32(255, 255, 255, 2555));
        }
    }
