...
bool ImGui::TabButtonEx(const char* label, const char* icon, const ImVec2& size_arg, ImGuiButtonFlags flags, bool selected, bool bugfix)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::TabButton(const char* label, const char* icon, const ImVec2& size_arg, bool selected, bool bugfix)
{
...
static std::map<ImGuiID, button_state> button_anim;
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    {
        button_anim.insert({ id, button_state() });
        it_anim = button_anim.find(id);
    }

    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImColor(37, 39, 41, 255) : hovered ? ImColor(27, 29, 31, 255) : ImColor(17, 19, 21, 255), g.IO.DeltaTime * 6.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, GetColorU32(scrollbar::bg), scrollbar::rounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min - ImVec2(2, 2), grab_rect.Max + ImVec2(2, 2), GetColorU32(scrollbar::scroll), scrollbar::rounding);
...
static std::map<ImGuiID, check_state> switch_anim;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    auto it_anim = switch_anim.find(id);

    if (it_anim == switch_anim.end())
    {
        switch_anim.insert({ id, check_state() });
        it_anim = switch_anim.find(id);
    }


    it_anim->second.circle_checkbox = ImLerp(it_anim->second.circle_checkbox, *v ? 19.f : 0.f, g.IO.DeltaTime * 8.f);
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? ImColor(255, 0, 0) : hovered ? ImColor(27, 29, 31, 255) : ImColor(17, 19, 21, 255), g.IO.DeltaTime * 8.f);
    it_anim->second.clip_mark = ImLerp(it_anim->second.clip_mark, *v ? checkmark : 0.f, g.IO.DeltaTime * 8.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? ImColor(255, 255, 255, 255) : hovered ? ImColor(180, 180, 180, 255) : ImColor(120, 120, 120, 255), g.IO.DeltaTime * 8.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRect(check_bb.Min + ImVec2(w - 20, 1), check_bb.Max + ImVec2(w, 1), ImColor(17, 19, 21, 255), 100.f, ImDrawFlags_None, 2.5f);

    GetWindowDrawList()->AddCircleFilled(check_bb.Min + ImVec2(w - 10 + it_anim->second.circle_checkbox, 10.2f), 5.5f, GetColorU32(it_anim->second.background), 100.f);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(21, 21, 21, 255));
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddRectFilled(bb.Min + ImVec2(GetStyle().WindowPadding.x, 0), bb.Max - ImVec2(GetStyle().WindowPadding.x, 0), ImColor(21, 21, 21, 255));
            if (g.LogEnabled)
...
bool ImGui::Tabs(bool selected, const char* ico, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? ImColor(255, 0, 0) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? ImColor(255, 255, 255, 255) : ImColor(180, 180, 180, 255), g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(0, (size_arg.y / 2.3f)), bb.Max, ImColor(0, 0, 0, 0), ImColor(0, 0, 0, 0), GetColorU32(it_anim->second.background), GetColorU32(it_anim->second.background));
    PopStyleVar();
...
static std::map<ImGuiID, combo_state> combo_anim;

extern ImFont* arrow;

bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    SetCursorPos(GetCursorPos() + ImVec2((GetContentRegionMax().x - style.WindowPadding.x) - w, 3));

    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 20.f));
    ...
    auto it_anim = combo_anim.find(id);

    if (it_anim == combo_anim.end())
    {
        combo_anim.insert({ id, combo_state() });
        it_anim = combo_anim.find(id);
    }
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? val * 2 * 13.5f : 0.f, g.IO.DeltaTime * 12.f);
    it_anim->second.background = ImLerp(it_anim->second.background, it_anim->second.opened_combo ? ImColor(255, 255, 255, 255) : hovered ? ImColor(180, 180, 180, 255) : ImColor(120, 120, 120, 255), g.IO.DeltaTime * 6.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);

    GetWindowDrawList()->AddRectFilled(bb.Min - ImVec2(0, 1), ImVec2(value_x2 + 27, bb.Max.y + 1), ImColor(17, 19, 21, 255), 2.f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.background));

    RenderTextClipped(bb.Min + ImVec2(5, -1), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);

    GetWindowDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(50, 1), ImVec2(value_x2, bb.Max.y + 1), ImColor(17, 19, 21, 0), ImColor(17, 19, 21, 255), ImColor(17, 19, 21, 255), ImColor(17, 19, 21, 0));

    PushFont(arrow);
    RenderTextClipped(bb.Min + ImVec2(w - 20, 2.f), ImVec2(value_x2 + 27, bb.Max.y), "H", NULL, NULL);
    PopFont();

    if (label_size.x > 0) RenderText(ImVec2(bb.Max.x - (GetContentRegionMax().x - style.WindowPadding.x), bb.Min.y - 1), label);
    ...
    PushStyleColor(ImGuiCol_WindowBg, ImVec4(ImColor(17, 19, 21, 255)));
...
static std::map<ImGuiID, slider_state> slider_anim;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    auto it_anim = slider_anim.find(id);

    if (it_anim == slider_anim.end())
    {
        slider_anim.insert({ id, slider_state() });
        it_anim = slider_anim.find(id);
    }

    it_anim->second.background = ImLerp(it_anim->second.background, hovered ? ImColor(27, 29, 31, 255) : ImColor(17, 19, 21, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? ImColor(255, 255, 255, 255) : hovered ? ImColor(180, 180, 180, 255) : ImColor(120, 120, 120, 255), g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 5), frame_bb.Max - ImVec2(0, 5), GetColorU32(it_anim->second.background), 4.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 5), frame_bb.Min + ImVec2(it_anim->second.slider_slow - 3, 10), ImColor(0, 255, 0), 4.f);
        GetWindowDrawList()->AddCircleFilled(ImVec2(frame_bb.Min.x + it_anim->second.slider_slow, grab_bb.Max.y - 6), 7.f, ImColor(0, 255, 0), 20.f);
    }
...
std::map<ImGuiID, input_state> inputtanim;
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2((GetContentRegionMax().x - style.WindowPadding.x) - w, 0));

    const ImVec2 frame_size = CalcItemSize(size_arg, w, (is_multiline ? g.FontSize * 8.0f : 24.f));
    ...
    auto it_anim = inputtanim.find(id);

    if (it_anim == inputtanim.end())
    {
        inputtanim.insert({ id, input_state() });
        it_anim = inputtanim.find(id);
    }

    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 2), frame_bb.Max + ImVec2(0, 1), GetColorU32(input::bg), input::rounding);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(5, 3);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(text::text_selected); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
        ...
        {
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, GetColorU32(is_displaying_hint ? text::text : text::text_active), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow, 3), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 3), GetColorU32(text::text));
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, GetColorU32(is_displaying_hint ? text::text : text::text_active), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? text::text_active : hovered ? text::text_hov : text::text, g.IO.DeltaTime * 6.f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.background));
    RenderText(ImVec2(frame_bb.Max.x - (GetContentRegionMax().x - style.WindowPadding.x), frame_bb.Min.y + 3), label);
    PopStyleColor();
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(17, 17, 17, 255)));
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) GetWindowDrawList()->AddText(window->DC.CursorPos - ImVec2(w_button, -1), ImColor(120, 120, 120, 255), label);

    ImFormatString(buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X", ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255));
    // GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(GetWindowContentRegionMax().x - 140, 1), GetColorU32(text::text), buf);

    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(120, 120, 120, 255)));
...
std::map<ImGuiID, picker_state> pickeranim;
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float bar0_pos_x = GetWindowPos().x + 12;
    ...
    // Render SV Square
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, picker::rounding, ImDrawFlags_RoundCornersTop);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, picker::rounding, ImDrawFlags_RoundCornersTop);
    ...
    auto it_anim = pickeranim.find(ImGui::GetID(label));

    if (it_anim == pickeranim.end())
    {
        pickeranim.insert({ ImGui::GetID(label), picker_state() });
        it_anim = pickeranim.find(ImGui::GetID(label));
    }
    ...
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, bar_pos.y + 4), ImVec2(bar0_pos_x + bars_width, bar_pos.y + sv_bar_size - 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, picker::rounding, ImDrawCornerFlags_Bot);
    ...
    if (alpha_bar)
    {
        ...
        //    GetForegroundDrawList()->AddRectFilled(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), ImColor(0, 0, 255, 0), s::picker::rounding_picker);
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 6.f);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(w - 3, 4), window->DC.CursorPos + size + ImVec2(w, -8));
    ItemSize(ImRect(bb.Min, bb.Max + ImVec2(0, 7)));
    ...
    float grid_step = ImMin(size.x, size.y) / 3.85f;
    ...
    else
    {
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), picker::rounding);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), picker::rounding);
    }

    if (IsRectVisible(bb_inner.Min, bb_inner.Max))
        GetForegroundDrawList()->AddShadowRect(bb_inner.Min + ImVec2(4, 4), bb_inner.Max - ImVec2(4, 4), GetColorU32(col_source), 20, ImVec2(0, 0), picker::rounding);
...
std::map<ImGuiID, select_state> selectanim;
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    //  if (hovered || selected)
    //  {
    //      RenderFrame(bb.Min, bb.Max, GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header), false, 0.0f);
    //  }
    //
    if (span_all_columns && window->DC.CurrentColumns)
        PopColumnsBackground();
    ...
    auto it_anim = selectanim.find(id);

    if (it_anim == selectanim.end())
    {
        selectanim.insert({ id, select_state() });
        it_anim = selectanim.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, selected ? ImColor(255, 0, 0) : hovered ? ImColor(180, 180, 180, 255) : ImColor(120, 120, 120, 255), g.IO.DeltaTime * 6.f);
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, ImGuiTabItem* src_tab, ImVec2 mouse_pos)
{
    ...
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
    {
        ...
        //GetForegroundDrawList()->AddRect(ImVec2(x1, tab_bar->BarRect.Min.y), ImVec2(x2, tab_bar->BarRect.Max.y), IM_COL32(255, 0, 255, 0));
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    if (out_text_clipped)
    {
        *out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_pixel_clip_bb.Max.x;
        //draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f, *out_text_clipped ? IM_COL32(255, 0, 255, 0) : IM_COL32(0, 255, 0, 255));
    }
