...
namespace shared::input
{
	...
	void init( HWND window )
	{
		...
		m_window = window;
		if ( !m_window )
			MessageBoxA( NULL, "input::init - Failed to find input window", "fatal error", NULL );

		m_original_wndproc = reinterpret_cast< WNDPROC >( SetWindowLongPtrW( m_window, GWLP_WNDPROC, reinterpret_cast< LONG_PTR >( hook ) ) );
		if ( !m_original_wndproc )
			MessageBoxA( NULL, "input::init - Failed to set new WndProc", "fatal error", NULL );
	}
	...
	LRESULT WINAPI hook( HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam )
	{
		...
		{
			gui::handle_input( hwnd, msg, wparam, lparam );
		...
		return CallWindowProcW( m_original_wndproc, hwnd, msg, wparam, lparam );
	}
}